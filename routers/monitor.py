...
def create_monitor(data: MonitorInput, id: str = Depends(checkuser), session = Depends(get_session)):
...
def read_monitors(id: str = Depends(checkuser), session = Depends(get_session)):
    monitors = session.exec(select(Monitor).where(Monitor.user_id == int(id))).all()
    return monitors

@router.delete("/delete/{monitor_id}")
@limiter.limit("5/minute")
def delete_monitor(monitor_id: int, id: str = Depends(checkuser), session = Depends(get_session)):
...
@limiter.limit("5/minute")
def update_monitor(monitor_id: int, data: MonitorInput, id: str = Depends(checkuser), session = Depends(get_session)):
    monitor = session.exec(select(Monitor).where(Monitor.id == monitor_id)).first()
    if not monitor:
        raise HTTPException(status_code=404, detail="Monitor not found")
